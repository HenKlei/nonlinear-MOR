...
class ColoredFormatter(logging.Formatter):
    ...
    `NONLINEAR_MOR_COLORS_DISABLE` environment variable to `1`.
    """

    def __init__(self):
        disable_colors = int(os.environ.get('NONLINEAR_MOR_COLORS_DISABLE', 0)) == 1
        if disable_colors:
...
def getLogger(module, level=None, filename=''):
    ...
    filename
        ...
        written to.
    """
    module = 'nonlinear_mor' if module == '__main__' else module
...
def set_log_levels(levels=None):
    ...
    """
    levels = levels or {'nonlinear_mor': 'INFO'}
