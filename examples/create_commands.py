...
with open("run_commands_in_screens.txt", "w") as f:
    for vals in itertools.product(example, spatial_shape, num_time_steps, additional_parameters,
                                  num_training_parameters, sampling_mode, reference_parameter, alpha, exponent, sigma,
                                  oversampling_size, optimization_method, max_reduced_basis_size, num_workers, l2_prod,
                                  neural_network_training_restarts,
                                  #hidden_layers,
                                  interval,
                                  #full_vector_fields_filepath_prefix,
                                  write_results):
#        e, nx, nt, ap, n_train, sm, rp, al, exp, si, os, opm, max_rb, num_w, pr, nntr, hl, i, fvvf, wr = vals
        e, nx, nt, ap, n_train, sm, rp, al, exp, si, os, opm, max_rb, num_w, pr, nntr, i, wr = vals
        f.write(f"source ../../venv/bin/activate && python perform_reduction.py {e} {nx} --num-time-steps {nt} "
                ...
                f"--reference-parameter {rp} --alpha {al} --exponent {exp} --sigma {si} "
